...
const Sidebar = ({ 
  ...
  setIsHighThinking: (v: boolean) => void
}) => {
  const menuItems = [
  ...
  return (
    <div className="hidden md:flex w-[280px] flex-col h-screen pt-8 pb-6 pl-8 pr-4">
      ...
      <nav className="flex-1 space-y-1.5 pr-2">
        {menuItems.map((item) => (
          <button
            ...
            className={cn(
              "w-full flex items-center gap-3 px-4 py-3 rounded-full text-[14px] font-medium transition-all text-left",
              activeTab === item.id 
      ...
      <div className="px-6 py-8 mt-auto border-t border-black/[0.04]">
        <div className="flex flex-col gap-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              <Sparkles size={14} className={cn("transition-colors", isHighThinking ? "text-emerald-500" : "text-zinc-400")} />
              <span className="text-[12px] font-semibold text-zinc-800 tracking-tight uppercase">High Thinking</span>
            </div>
            <button 
              onClick={() => setIsHighThinking(!isHighThinking)}
              className={cn(
                "relative inline-flex h-5 w-9 shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none",
                isHighThinking ? "bg-black" : "bg-zinc-200"
              )}
            >
              <span 
                className={cn(
                  "pointer-events-none inline-block h-4 w-4 transform rounded-full bg-white shadow ring-0 transition duration-200 ease-in-out",
                  isHighThinking ? "translate-x-4" : "translate-x-0"
                )}
              />
            </button>
          </div>
          <p className="text-[10px] text-zinc-400 leading-relaxed font-medium">
            Elevates reasoning for complex planning and multi-step execution tasks.
          </p>
        </div>
      </div>
    </div>
